...
interface ChatPanelProps extends Record<string, never> {
...
export default function ChatPanel(_props: ChatPanelProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Use OpenRouter with Claude 3.7 model
      const response = await fetch('/.netlify/functions/openrouter-proxy', {
        ...
        body: JSON.stringify({
          ...
          temperature: 0.7,
          max_tokens: 1000,
          system: 'You are an AI assistant helping users find and plan events and dates. Extract event details and provide specific recommendations.'
        })
      });
