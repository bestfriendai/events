...
export default function SearchPanel({ events, onEventSelect, onFilterChange }: SearchPanelProps) {
  ...
  const [currentFilters, setCurrentFilters] = useState<Filter>({
    // Initialize with default/empty values based on Filter type
    category: undefined,
    dateRange: undefined,
  ...
  // Handler for individual filter changes from FilterPanel
  const handleIndividualFilterChange = (key: string, value: string | number | string[]) => {
    const updatedFilters = { ...currentFilters, [key]: value };
    setCurrentFilters(updatedFilters);
    // Propagate the complete updated filter object upwards
  ...
  return (
    <div className="h-full bg-white p-6 overflow-auto">
      ...
      <div className="space-y-4">
        ...
        {showFilters && (
          <FilterPanel filters={currentFilters} onFilterChange={handleIndividualFilterChange} />
        )}
