...
class AIMonitor {
  ...
  startRequest(service: 'claude'): number {
    return Date.now();
  }

  endRequest(service: 'claude', startTime: number, success: boolean, error?: string) {
  ...
  getErrorRate(service: 'claude'): number {
  ...
  getAverageLatency(service: 'claude'): number {
